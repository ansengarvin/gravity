...
uniform highp int horizontal; // 1 for horizontal blur, 0 for vertical blur

highp float weight[5] = float[] (0.227027, 0.1945946, 0.1216216, 0.054054, 0.016216);

void main() {
    highp vec2 tex_offset = 1.0 / vec2(textureSize(image, 0));
    highp vec3 result = texture(image, TexCoords).rgb * weight[0];
    if (horizontal == 1) {
        for (int i = 1; i < 5; ++i) {
